...
**The exemption covers the blocker-resolved wake too, not just the reconciler flip — so a declared park outlives its own blocker edge.** The same suppression gates the eager status recompute and both blocker-resolved wakes (the edge-triggered fast path and the periodic resolved-blocker sweep), exactly as an executive hold does. So if an issue carries *both* a real `blockedBy` edge and a declaration, closing that blocker neither wakes the assignee nor resumes the row: the declared human gate is still live, and the structural blocker clearing says nothing about it. This is deliberate — but it means such a row sits `blocked` with zero unresolved blockers and **no wake path at all**, which `isDeadEndBlocked` also declines to flag. Its only surface is the blocked inbox, which lists it as `external_wait` with the declared owner and action. A human-only gate does not move on a timer, so there is nothing to poll; **the corollary is that a stale declaration parks the row just as effectively as a live one, and deleting the two lines when the gate clears is the declaring agent's job.** Removing them puts the row straight back on the wake path.
