...
- source-scoped recovery routing is cause-keyed: lost processes, transient infrastructure failures, missing successful-run dispositions, and output-inactivity terminations retry the original agent when invokable; dependency-blocked work keeps its assignee while waiting for blockers and uses a bounded 12-attempt (roughly ten-hour) retry horizon before handing off to dependency recovery; provider-quota failures create/reuse a scheduled wait-recovery monitor without a takeover wake; workspace validation and unknown causes route to the manager ladder
