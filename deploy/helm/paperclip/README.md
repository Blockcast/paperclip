...
| githubApp | object | `{"appIdKey":"app_id","enabled":false,"installationIdKey":"installation_id","prReviewGateStatusContext":"","privateKeyKey":"private_key.pem","reviewGateCaptureEnabled":false,"reviewGateEnabled":false,"reviewGateExpectedAppId":"","reviewGateExpectedInstallationId":"","reviewGateRepositories":[],"reviewerBotLogin":"","secretName":"paperclip-github-app-creds"}` | GitHub App creds for server-side installation-token minting. Lets the API authoritatively verify (via the GitHub API) that a PR-review run actually left a review/comment before completing it (BLO-10448). Deployments that run the PR reviewer must enable this; missing credentials warn at API startup and fail review completion closed. Disabled by default for deployments that do not use the reviewer workflow. |
...
| githubApp.prReviewGateStatusContext | string | `""` | Commit-status context to FAIL when a PR-review run exhausts its automatic retries (e.g. "review/ally-complete"). A required status that is never posted reads as "Expected — waiting for status" forever, so an exhausted reviewer chain silently wedges the PR. Empty (default) -> the server posts no status. Requires the App to hold `statuses: write` on the target repos. |
| githubApp.reviewGateCaptureEnabled | bool | `false` | Durably capture signed review-gate webhooks without processing them. Roll this out to every API replica before enabling authority. |
| githubApp.reviewGateEnabled | bool | `false` | Process captured review-gate deliveries and dispatch reconciliation. Requires capture; the Blockcast release workflow also proves the live capture rollout completed before promotion. |
| githubApp.reviewGateExpectedAppId | string | `""` | GitHub App identity allowed to produce the protected review-gate status. |
| githubApp.reviewGateExpectedInstallationId | string | `""` | Installation identity allowed to produce the protected review-gate status. |
| githubApp.reviewGateRepositories | list | `[]` | Repositories whose signed webhook deliveries drive review-gate revocation and reconciliation. |
...
| prometheusRule | object | `{"additionalGroups":[],"agentJobNameRegex":"ac-.*","backoffLimitExceededFor":"2m","backoffLimitExceededWindow":"5m","enabled":false,"githubDeliveryDeadLetterFor":"5m","githubDeliveryDeadLetterWindow":"30m","githubDeliverySuppressionFor":"5m","githubDeliverySuppressionOutageCauses":"heartbeat.scheduling_suppressed|dispatch_rejected|other","githubDeliverySuppressionWindow":"30m","labels":{},"namespace":"","processLostElevatedFor":"10m","processLostLivenessNullWindowCount":5,"processLostPageFor":"2h","processLostPagePerDay":40,"processLostSignalBlindFor":"15m","processLostWarnPerDay":20,"unschedulableFor":"15m","unschedulablePodThreshold":0,"wakeTerminalFailedAgeSeconds":1800,"wakeTerminalFailedFor":"5m","wakeTerminalFailedRunbookUrl":"https://github.com/Blockcast/paperclip/blob/master/runbooks/agent-wakeup-terminal-failed.md","zeroTokenFor":"2m","zeroTokenStreakThreshold":3}` | Prometheus Operator rules for Paperclip runtime health. Requires the `monitoring.coreos.com/v1` CRDs and the ServiceMonitor scrape above. Disabled by default so the chart still installs on clusters without the operator. |
...
| prometheusRule.githubDeliveryDeadLetterFor | string | `"5m"` | How long a non-zero dead-letter rate must persist before firing. Kept short: a dead-lettered delivery is already terminal, so there is nothing to self-resolve. |
| prometheusRule.githubDeliveryDeadLetterWindow | string | `"30m"` | Rolling window for the GitHub review-request dead-letter rate (BLO-18859). Any dead-lettered delivery in this window fires; the counter is flat at zero in steady state. |
| prometheusRule.githubDeliverySuppressionFor | string | `"5m"` | How long outage-like suppression must persist before firing. Same reasoning as the dead-letter alert: suppression is terminal, nothing self-resolves. |
| prometheusRule.githubDeliverySuppressionOutageCauses | string | `"heartbeat.scheduling_suppressed|dispatch_rejected|other"` | Prometheus regex of suppression causes that mean an outage rather than a deliberate decline (BLO-18859). Expected policy declines (company.inactive, heartbeat.cooldown.active, heartbeat.disabled, heartbeat.wakeOnDemand.disabled, budget.blocked, agent.not_invokable, heartbeat.worktree_execution_cutoff, issue_tree_hold_active) are excluded so they never page. `other` is included: it is the bucket for a skip reason missing from KNOWN_GITHUB_SUPPRESSION_CAUSES, and an untriaged cause is not a known-good decline. |
| prometheusRule.githubDeliverySuppressionWindow | string | `"30m"` | Rolling window for outage-like terminal suppression of GitHub review-request wakes (BLO-18859). |
...
| workerProbes | object | `{"liveness":{"failureThreshold":6,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1:3100"}],"path":"/healthz","port":"http"},"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":10},"readiness":{"failureThreshold":3,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1:3100"}],"path":"/api/health","port":"http"},"initialDelaySeconds":5,"periodSeconds":20,"timeoutSeconds":20},"startup":{"failureThreshold":30,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1:3100"}],"path":"/healthz","port":"http"},"periodSeconds":10,"timeoutSeconds":5}}` | Worker StatefulSet probe configuration. The worker serves the same `/healthz` endpoint as the API tier; probing HTTP prevents Kubernetes from routing API-to-worker requests before the worker is actually listening. |
| workerProbes.liveness | object | `{"failureThreshold":6,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1:3100"}],"path":"/healthz","port":"http"},"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":10}` | Worker liveness probe.  BLO-19722 — why this is deliberately laxer than `probes.liveness` above.  What `/healthz` actually measures: "is the event loop able to service a request right now", and nothing else. It says nothing about database or heartbeat health, and that is deliberate — see below.  BLO-32164 update (2026-09-10). This used to read "nothing declares this route": the request fell through to the SPA catch-all, which answered 200 with the UI shell after a synchronous `readFileSync` of index.html. There is now an explicit handler (`app.get("/healthz")`, mounted beside /metrics ahead of httpLogger, the hostname guard and actorMiddleware). The measured semantics are unchanged; what changed is that the probe no longer does blocking disk I/O on the path it is timing, no longer depends on the UI existing, and no longer needs the `Host: 127.0.0.1:3100` header to clear the private-hostname allowlist. `server/src/__tests__/healthz-probe-route.test.ts` pins the route against the probe paths in this file so the two cannot drift apart again.  The "latent landmine" previously recorded here — a build serving no `ui-dist` skips the catch-all, so every probe 404s and CrashLoops the worker for a reason unrelated to its health — is closed by that handler.  At the previous `timeoutSeconds: 5 / periodSeconds: 30 / failureThreshold: 3` a ~90s event-loop stall was enough for kubelet to kill the pod. That is not theoretical: on 2026-07-31 `Liveness probe failed: context deadline exceeded` on `paperclip-0` was followed ~5s later by every active `agent-opencode-*` pod dying with it. A busy heartbeat pass is not a wedged process, but this probe could not tell them apart.  The asymmetry with the API tier is the point. An API replica is stateless and horizontally scaled, so killing one early is cheap. The worker is a singleton supervising every in-flight agent run, so a false kill orphans all of them — the exact failure this issue exists to stop. We therefore buy stall tolerance at the cost of slower detection of a true wedge: 6 × 30s ≈ 180s before a kill, versus ~90s previously.  Still not fixed, and now the dominant remaining cause: making `/healthz` un-blockable. Node runs our request handling on one thread, so any in-process endpoint is blocked by whatever blocks the loop; a genuinely independent health responder needs a worker thread (or a sidecar). The explicit handler removes the probe's *own* blocking work but cannot protect it from someone else's. Measured 2026-09-10 on `paperclip-0`: `exitCode: 137` at 08:09:25Z with memory steady at ~3 GiB against a 10 GiB limit (OOM excluded), i.e. a liveness kill at 6 × 30s — the widened threshold above bought time but did not remove the cause. Tracking as BLO-32164 (a); the stall's own mechanism is BLO-32668.  Do NOT add a database check to this probe. BLO-32164 originally proposed exactly that; measured 2026-09-10, `/api/health` took 13.19s under pool saturation while `/healthz` answered in 6ms in the same second. A query here would import that stall onto the liveness path and kill the singleton worker for pool saturation, converting a latency problem into an outage. |
| workerProbes.readiness | object | `{"failureThreshold":3,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1:3100"}],"path":"/api/health","port":"http"},"initialDelaySeconds":5,"periodSeconds":20,"timeoutSeconds":20}` | Worker readiness probe.  BLO-31945 — why this is no longer tuned "tighter than liveness".  The rationale here used to read: "Keep it tighter than liveness so API callers stop routing to a worker whose event loop is not answering promptly." That holds for a horizontally-scaled tier, where readiness sheds traffic to healthy peers. This StatefulSet runs `replicas: 1`, so there is no peer to shed to: removing the only endpoint does not redirect plugin traffic, it 502s it. Every API-to-worker call then fails in the `catch` at `server/src/routes/worker-tier-proxy.ts` with "Worker tier unreachable", which is how a `/healthz` latency blip became a total plugin-tier outage — Alertmanager webhook delivery included. Alertmanager then retries each delivery 4x into the worker that just failed its probe.  Measured on `paperclip-0` over the 24h to 2026-09-05T20:30Z from the kubelet `prober_probe_*` metrics — same endpoint, same pod, same window, with the per-probe timeout as the only material difference: readiness at a 5s timeout failed 100.2 of 8559 probes (1.171%), while liveness at a 10s timeout failed 13.1 of 2838 (0.462%).  Readiness failed at 2.53x the liveness rate against the identical `/healthz`. The tail latency confirms the cause: p99.9 of *successful* liveness probes is 8.59s, i.e. `/healthz` legitimately takes longer than the old 5s readiness timeout. A timeout set below the endpoint's own tail is a false-positive generator, not a sensitivity knob.  Trap when re-deriving this: `prober_probe_duration_seconds` is a SUCCESS-ONLY histogram — its `_count` matches `prober_probe_total{result="successful"}`, not the total. Timed-out probes are censored out of it, so the bucket distribution on its own reads as "almost nothing exceeds 5s" and will invert this diagnosis if used without the failure counter.  Superseded by BLO-35948 below: the endpoint moved to `/api/health`, and the figures are now a 20s timeout and 20s period, probes at t=0/20/40, and the third consecutive failure at t=60s. The derivation, as first written: timeoutSeconds now matches liveness (10s), above the measured tail, and periodSeconds moves to 15 so a probe cannot outlast its own period. Worst-case endpoint removal, stated derivably rather than as `3 x period`: probes start at t=0/15/30 and each times out 10s in, so the third consecutive failure is only observed at t=40s, and endpoint propagation adds more on top. That is ~40s+ versus ~20s+ under the old 10s/5s settings. The slower detection costs nothing real at `replicas: 1`, because endpoint removal has no upside here in the first place.  Decision, recorded explicitly rather than left implicit: readiness is KEPT, not retired. It still gates the endpoint during rollout, and deleting it would make a genuine wedge hang API callers until TCP timeout instead of failing fast. What is retired is tuning it as though it could shed load. This is mitigation, not a cure: at the liveness failure rate the probe still drops the endpoint occasionally, and the durable fix is a second replica (BLO-29307 / BLO-29004) or a `/healthz` that cannot be blocked by the event loop. Until one of those lands, this probe must be tolerant rather than tight.  `failureThreshold: 3` is load-bearing, not incidental, and it is the knob a future reader will reach for once told not to touch the timeout. At the 0.462% per-probe failure rate this endpoint actually exhibits, requiring three *consecutive* failures puts an endpoint drop at roughly one per several years; `failureThreshold: 1` would drop the only endpoint on any single tail-latency blip, i.e. tens of times a day, reopening exactly the outage this change closes. `tests/probes.test.mjs` enforces both this floor and the timeout relationship while the tier is single-replica.  BLO-35948 (2026-09-24) — why this one probe targets `/api/health` while liveness and startup stay on `/healthz`.  `/healthz` answers "can the event loop service a request", and nothing else. On 2026-09-24 the worker wedged for 2h15m on 9 postgres.js connections stuck in `ClientRead`: the pool was dead, every API-to-worker call returned `Connect Timeout Error (paperclip-workers:3100)` / 502 / 504, and the pod read `1/1 Ready` the entire time because `/healthz` was still answering. There was no Kubernetes signal at all. `/api/health` executes `SELECT 1` through the same pool (plus, in authenticated mode, the bootstrap role count and, while bootstrap is pending, an invite count) and 503s if any of them fails; it is the only probe-reachable route that exercises the path that actually hung. All of those queries sit inside the handler's one guard (`server/src/routes/health.ts`, pinned by `server/src/__tests__/health.test.ts`), and the probe carries no session cookie or tenant token, so `actorMiddleware` adds no query of its own.  The split is the whole point. Liveness and startup must NOT do this: a query on the liveness path imports pool latency into a kill decision and restarts the singleton worker — with every in-flight agent run on it — for what is a saturation problem. See the liveness block above and `server/src/app.ts`, whose own comment already prescribed this placement: "Readiness gating on the database belongs in `/api/health`."  Tolerances, derived rather than inherited. BLO-31945's rule holds and is simply re-applied to the new endpoint: a timeout below the endpoint's own tail is a false-positive generator. `/api/health` has a different, much longer tail than `/healthz` — measured 2026-09-10 under pool saturation it took 13.19s while `/healthz` answered in 6ms in the same second. So the 10s timeout that was correct for `/healthz` would fail on a merely saturated pool. timeoutSeconds is therefore 20s, ~1.5x that worst recorded non-wedge sample, and periodSeconds follows it so a probe cannot outlast its own period. Healthy-pod control, 15 samples through the Service at 2026-09-24T16:2xZ: 11.7ms-69.8ms, i.e. ~300x of headroom. `failureThreshold` stays at BLO-31945's floor of 3 — three *consecutive* failures, so a single blip cannot drop the only endpoint.  Bounded time to NotReady, stated derivably rather than as `3 x period`: probes fire at t=0/20/40 and each times out 20s in, so the third consecutive failure is observed at t=60s, plus endpoint propagation.  The trade-off, recorded as a decision (BLO-35948 AC4) and not left implicit: this tier has exactly ONE endpoint, so readiness failure does not shed load, it removes the Service's only backend and 502s plugin traffic. That cost is accepted here because in the wedge state those calls are ALREADY failing — the change makes them fail immediately instead of after a 10s connect timeout, and it fires the existing `PaperclipWorkerTierNoReadyEndpoints` critical alert, which is the signal the outage lacked. The genuinely new risk is the merely-slow case, and that is what the 20s timeout exists to exclude. Known circularity: while the worker is NotReady, Alertmanager webhook delivery into Paperclip is down too — but during a real wedge it is down anyway, so this changes nothing in the case that matters. The durable fix for all of it remains a second replica (BLO-29307 / BLO-29004). |
| workerProbes.startup | object | `{"failureThreshold":30,"httpGet":{"httpHeaders":[{"name":"Host","value":"127.0.0.1:3100"}],"path":"/healthz","port":"http"},"periodSeconds":10,"timeoutSeconds":5}` | Worker startup probe.  Deliberately left at `timeoutSeconds: 5`, below the 8.59s `/healthz` tail measured above, and that exemption is intentional rather than an oversight: `failureThreshold: 30` at a 10s period gives a ~300s budget, so an individual timed-out probe is absorbed instead of being fatal. The readiness defect was that three false timeouts were *sufficient* to remove the endpoint; here they are not close to sufficient, so tail sensitivity carries no cost. Re-tighten `failureThreshold` here and that stops being true. |
