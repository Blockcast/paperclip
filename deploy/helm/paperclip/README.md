...
| githubApp.reviewerBotLogin | string | `""` | GitHub login of the reviewer bot used to filter reviews/comments. Empty -> server default ("allyblockcast[bot]"). |
...
| workerProbes | object | `{"liveness":{"exec":{"command":["/bin/sh","-c","grep -qa 'server/dist/index.js' /proc/[0-9]*/cmdline"]},"failureThreshold":3,"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":2},"readiness":{"exec":{"command":["/bin/sh","-c","grep -qa 'server/dist/index.js' /proc/[0-9]*/cmdline"]},"failureThreshold":3,"initialDelaySeconds":5,"periodSeconds":10,"timeoutSeconds":2},"startup":{"exec":{"command":["/bin/sh","-c","grep -qa 'server/dist/index.js' /proc/[0-9]*/cmdline"]},"failureThreshold":30,"periodSeconds":10,"timeoutSeconds":2}}` | Worker StatefulSet probe configuration. The worker role does not bind the HTTP port reliably during startup, but it does run the same Node entrypoint; use a process check for the single StatefulSet pod and keep the API tier on `probes.*` HTTP checks. |
| workerProbes.liveness | object | `{"exec":{"command":["/bin/sh","-c","grep -qa 'server/dist/index.js' /proc/[0-9]*/cmdline"]},"failureThreshold":3,"initialDelaySeconds":30,"periodSeconds":30,"timeoutSeconds":2}` | Worker liveness probe. |
| workerProbes.readiness | object | `{"exec":{"command":["/bin/sh","-c","grep -qa 'server/dist/index.js' /proc/[0-9]*/cmdline"]},"failureThreshold":3,"initialDelaySeconds":5,"periodSeconds":10,"timeoutSeconds":2}` | Worker readiness probe. |
| workerProbes.startup | object | `{"exec":{"command":["/bin/sh","-c","grep -qa 'server/dist/index.js' /proc/[0-9]*/cmdline"]},"failureThreshold":30,"periodSeconds":10,"timeoutSeconds":2}` | Worker startup probe. |
