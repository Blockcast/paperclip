...
type PrReviewerQueryRunner = <T>(query: () => PromiseLike<T>) => Promise<T>;

async function runPrReviewerSelectionQuery<T>(
  query: () => PromiseLike<T>,
  runner?: PrReviewerQueryRunner,
): Promise<T> {
  return runner ? runner(query) : await query();
}
...
//
// BLO-21582 review follow-up: this is one request-wide budget (passed to
// withPrReviewerTaskLock as an absolute deadline, not re-armed per attempt),
// and it covers pool reservation, transaction setup, and every advisory-lock
// probe. A fallback read gets a separate small bound so a saturated pool cannot
// make the timeout path unbounded either.
...
class PrReviewerTaskLockTimeoutError extends Error {
  constructor(taskKey?: string) {
    super(
      taskKey
        ? `timed out acquiring PR reviewer task assignment lock for ${taskKey}`
        : "timed out acquiring PR reviewer task assignment lock",
    );
...
async function selectPrReviewerAgentId(
  ...
  taskKey: string,
  queryRunner?: PrReviewerQueryRunner,
): Promise<string | null> {
  if (configuredAgentIds.length === 0) return null;

  const activeRows = await runPrReviewerSelectionQuery(
    () =>
      db
        .select({ id: agents.id })
        .from(agents)
  ...
  const loadRows = await runPrReviewerSelectionQuery(
    () =>
      db
...
async function findActivePrReviewerForTask(
  ...
  return runPrReviewerSelectionQuery(
    () =>
      db
...
// `statement_timeout` is per statement, while both callers below receive an
// absolute request deadline. Re-arm it immediately before each blocking SQL
// statement so time already spent in BEGIN or an earlier fallback query is not
// granted again to the next statement.
async function applyRemainingStatementTimeout(
  reservation: ReservedPgConnection,
  deadline: number,
): Promise<boolean> {
  const remainingMs = Math.floor(deadline - Date.now());
  if (remainingMs <= 0) return false;
  await reservation.unsafe(`set statement_timeout = ${Math.max(1, remainingMs)}`);
...
async function withPrReviewerTaskLock<T>(
  db: Db,
  taskKeys: readonly string[] | string,
  deadline: number,
  action: (tx: PrReviewerSelectionDb) => Promise<T>,
): Promise<T> {
  const client = db.$client;
  const lockKeys = [...new Set(typeof taskKeys === "string" ? [taskKeys] : taskKeys)].sort();
  if (lockKeys.length === 0) throw new Error("PR reviewer task lock requires at least one key");
  while (true) {
    const remainingMs = deadline - Date.now();
    if (remainingMs <= 0) {
      throw new PrReviewerTaskLockTimeoutError(lockKeys[0]);
    }
    ...
    try {
      ...
      try {
        if (!(await applyRemainingStatementTimeout(reservation, deadline))) {
          throw new PrReviewerTaskLockTimeoutError(lockKeys[0]);
        }
        ...
        for (const lockKey of lockKeys) {
          ...
          const rows = await reservation`select pg_try_advisory_xact_lock(hashtextextended(${lockKey}, 0)) as acquired`;
...
// lock, a slow plan -- not just pool contention). A session-scoped `SET
// statement_timeout` installed before `BEGIN` does that at the database
// itself instead of merely abandoning the JS promise, so transaction setup is
// bounded too and nothing is left running once this connection is released.
// `read` may issue more than one statement (see `selectPrReviewerAgentId`'s
// two-query fallback), so it receives a runner that re-arms statement_timeout
// from the same absolute deadline before every query. A timeout here must
// propagate as "unknown", not as a false "confirmed no active reviewer" -- the
// caller has to keep that distinction to avoid silently swallowing a real loss
// just because the recheck itself couldn't finish in time.
async function boundedFallbackRead<T>(
  client: PgClient,
  deadlineMs: number,
  read: (reservedDb: PrReviewerSelectionDb, runQuery: PrReviewerQueryRunner) => Promise<T>,
): Promise<T | typeof FALLBACK_RECHECK_TIMED_OUT> {
  const remainingMs = deadlineMs - Date.now();
  const reservation = await reserveConnectionOrTimeout(client, remainingMs);
  if (reservation === RESERVATION_TIMED_OUT) return FALLBACK_RECHECK_TIMED_OUT;
  try {
    // `SET LOCAL` cannot protect BEGIN because it only takes effect after the
    // transaction exists. Set the session value first so backend stalls while
    // starting the transaction receive the same deadline, then reset it before
    // releasing the reserved connection. The interpolated value is our own
    // computed integer, never external input.
    if (!(await applyRemainingStatementTimeout(reservation, deadlineMs))) {
      return FALLBACK_RECHECK_TIMED_OUT;
    }
    await reservation`begin`;
    const runQuery: PrReviewerQueryRunner = async (query) => {
      if (!(await applyRemainingStatementTimeout(reservation, deadlineMs))) {
        throw FALLBACK_RECHECK_TIMED_OUT;
      }
      return await query();
    };
    const result = await read(drizzleOverReservedConnection(client, reservation), runQuery);
    if (!(await applyRemainingStatementTimeout(reservation, deadlineMs))) {
      throw FALLBACK_RECHECK_TIMED_OUT;
    }
    await reservation`commit`;
    ...
    await reservation`rollback`.catch(() => {});
    if (error === FALLBACK_RECHECK_TIMED_OUT || findPgError(error)?.code === "57014") {
      return FALLBACK_RECHECK_TIMED_OUT;
    }
    throw error;
  } finally {
    // The session setting was installed before BEGIN, so it survives COMMIT.
    // Always clear it before this physical connection returns to the pool.
    await reservation.unsafe(`set statement_timeout = 0`).catch(() => {});
    reservation.release();
  }
}
...
export function githubWebhookRoutes(db: Db, config: GithubWebhookConfig) {
  ...
  router.post("/", async (req, res) => {
    ...
    const reviewerWakeFired = await (async () => {
      ...
      try {
        const dispatchReviewerWake = async (tx: PrReviewerSelectionDb) => {
        ...
        try {
          ...
          // A lock timeout must never dispatch the wake outside the lock: the
          // issue-create duplicate guard and reviewer wake use the same PR
          // serialization namespace. A bounded, read-only recheck is safe for
          // the one case where a concurrent delivery already committed the
          // equivalent request, and avoids asking GitHub to retry a delivery
          // that is already durable. It is deliberately positive-only: an
          // absent or timed-out read is not evidence that it is safe to write.
          ...
          if (equivalentWake && equivalentWake !== FALLBACK_RECHECK_TIMED_OUT) {
            logger.info(
              {
              ...
              "github webhook reviewer wake lock exhausted, but an equivalent wake is already durable: asking no retry",
            );
            return false;
          }
          ...
          logger.warn(
            {
              ...
              prNumber: context.prNumber,
              repoFullName: context.repoFullName,
              fallbackRecheckTimedOut: equivalentWake === FALLBACK_RECHECK_TIMED_OUT,
            },
...
export const __test_boundedFallbackRead = boundedFallbackRead;
