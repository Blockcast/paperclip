...
function buildDependabotAlertIssueBody(input: {
  ...
  return [
    ...
    "- Dismissal path: a documented dismissal reason and a terminal dismissal webhook receipt are recorded on this issue. Direct observation of the GitHub alert state is sufficient but not required.",
    "",
    "## Verifying signal",
    ...
    `1. The remediation PR merges into the default branch of \`${repoFullName}\`, AND the default-branch manifest${alert.manifestPath ? ` \`${alert.manifestPath}\`` : ""} resolves ${alert.packageName ?? "the dependency"} at ${alert.patchedVersion ?? "a patched version"} or newer, with advisory ${advisory} cited in the evidence.`,
    `2. ${alertUrl} shows \`state: fixed\` for advisory ${advisory}.`,
    `3. A terminal webhook receipt on this issue, or ${alertUrl}, shows \`state: dismissed\` for advisory ${advisory}, together with the documented dismissal reason.`,
    "",
