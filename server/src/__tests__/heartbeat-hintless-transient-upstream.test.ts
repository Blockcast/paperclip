...
describeEmbeddedPostgres("hint-less gateway 503 does not strand", () => {
  ...
  afterAll(async () => {
    ...
    // The shared ARC runner can take longer than the global 120s hook limit
    // to drain heartbeat work, cascade the embedded database, and stop it.
  }, 240_000);
  ...
  it("escalates allocation_missing only when the immediate predecessor had the same fault", async () => {
    ...
    // "failed", so assert with .poll() instead of a synchronous read.
    ...
    // Give the post-finalization scheduler write time to surface any accidental
    // retry, rather than passing before that asynchronous write can occur.
    await new Promise((resolve) => setTimeout(resolve, 1_000));
