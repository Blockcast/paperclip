...
describeEmbeddedPostgres("accepted plan workspace refresh", () => {
  ...
  it("realizes an isolated workspace and drops stale shared task-session params before executing", async () => {
    ...
    const heartbeat = createHeartbeat({ penstockAvailabilityGate: allowPenstockGate });
  ...
  it("keeps accepted-plan children strategy-only until first realization after the base ref moves", async () => {
    ...
    adapterExecute.mockImplementationOnce(async () => ({
      ...
      resultJson: { summary: "Realized the planning source workspace." },
      provider: "test",
      model: "test-model",
    }));
    ...
    await vi.waitFor(async () => {
      ...
      if (latest && !["queued", "running"].includes(latest.status) && latest.status !== "succeeded") {
        throw new Error(
          `Planning source run failed: ${latest.errorCode ?? "unknown"}: ${latest.error ?? "unknown"} ${JSON.stringify(latest.resultJson)}`,
        );
      }
    ...
    adapterExecute.mockImplementationOnce(async (input) => {
      ...
      return {
        ...
        resultJson: { summary: "Child realized from the moved base." },
        provider: "test",
        model: "test-model",
      };
    });
