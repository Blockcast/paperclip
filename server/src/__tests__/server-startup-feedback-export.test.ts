...
vi.mock("../services/index.js", () => ({
  EXTERNAL_LIFECYCLE_COLD_BOOT_REATTACH_GRACE_MS: 0,
...
describe("startServer feedback export wiring", () => {
  ...
  it("runs the periodic stale-lock sweep independently of other recovery failures", async () => {
    ...
    try {
      ...
      // `reconcileDetachedQueuedRuns` is awaited *after* the sweep in the tick,
      // so it needs its own wait rather than a bare assertion on the same turn.
      await vi.waitFor(() =>
