...
describe("production Dockerfile k8s adapter runtime pins", () => {
  ...
  it("vendors the opencode_k8s adapter commit with runtime isolation, the env-dump deny, and Opus 5", () => {
    ...
    expect(serverDockerfile).toContain("preserve successful");
    expect(serverDockerfile).toContain("finite-timeout runs");
    expect(serverDockerfile).toContain("LOG_EXIT_COMPLETION_GRACE_MS");
