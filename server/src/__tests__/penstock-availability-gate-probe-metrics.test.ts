...
describe("penstock availability gate probe instrumentation (BLO-29900)", () => {
  ...
  it("books a fail-open auth fault as its own outcome, not as inconclusive", async () => {
    const fetchMock = vi
      .fn()
    ...
    expect(await probeCount({ path: "capacity", outcome: "auth_fault" })).toBe(1);
    expect(await probeCount({ path: "capacity", outcome: "inconclusive" })).toBe(0);
    expect(await probeCount({ path: "capacity", outcome: "deny_temporary" })).toBe(0);
    // It still drives the fallback, so it still belongs in the denominator.
    expect(await observedPaths()).toEqual(["capacity", "messages_fallback"]);
  });

  it("books a fail-open auth fault on the messages path too, not as a healthy ping", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(new Response("", { status: 404 }))
    ...
    expect(await probeCount({ path: "messages_fallback", outcome: "auth_fault" })).toBe(1);
    // Must not read as a successful ping: `ok` on this path is the series that
    // says provider inference quota was actually spent and answered.
    expect(await probeCount({ path: "messages_fallback", outcome: "ok" })).toBe(0);
    expect(await probeCount({ path: "messages_fallback", outcome: "error" })).toBe(0);
  });
