...
describeEmbeddedPostgres("github-webhook route", () => {
  ...
  function buildApp(config: Pick<GithubWebhookConfig, "prReviewerAgentIds" | "prReviewerAgentId" | "prReviewerBotLogin" | "selfReviewEscalationThreshold" | "dependabotAgentId" | "dependabotMinSeverity" | "heartbeatOptions" | "pullRequestStateResolver"> = {}) {
    const app = express();
  ...
  describe("pull_request work products", () => {
    ...
    it("reconciles a true same-second close-then-reopen against GitHub's current state", async () => {
      // GitHub's `pull_request.updated_at` is second-granular, so a rapid
      // close-then-reopen can carry one timestamp. The action rank cannot tell
      // whether it is this order or reopen-then-close; the current GitHub PR
      // state is the only authoritative tie-break.
      const { issueId } = await seedIssueWithIdentifier("BLO-40020");
      const app = buildApp({
        pullRequestStateResolver: async () => ({
          state: "open",
          merged: false,
          draft: false,
          title: "Reopened current state",
          url: "https://github.com/Blockcast/paperclip/pull/4260",
          headSha: "same-second-reopen-head",
          branch: "fix/blo-40020",
          mergedAt: null,
          updatedAt: "2026-04-30T10:00:00Z",
        }),
      });
    ...
    it("fails closed for productivity when same-second state reconciliation is unavailable", async () => {
      const { issueId } = await seedIssueWithIdentifier("BLO-40025");
      const app = buildApp({
        pullRequestStateResolver: async () => ({ error: "github_unavailable" }),
      });
      ...
      await postPr(app, prPayload({
        ...
        merged: false,
        updatedAt: "2026-04-30T10:00:00Z",
      }), "wp-same-second-pending-1");
      await postPr(app, prPayload({
        action: "reopened",
        identifier: "BLO-40025",
        number: 4265,
        headSha: "same-second-pending-head",
        updatedAt: "2026-04-30T10:00:00Z",
      }), "wp-same-second-pending-2");
      ...
      expect(rows[0]?.metadata).toMatchObject({ sourceStateVerification: "pending" });
    });
    ...
    it("reconciles a true same-second reopen-then-close against GitHub's current state", async () => {
      ...
      const app = buildApp({
        pullRequestStateResolver: async () => ({
          ...
          merged: false,
          draft: false,
          title: "Closed current state",
          url: "https://github.com/Blockcast/paperclip/pull/4263",
          headSha: "same-second-closed-head",
          branch: "fix/blo-40023",
          mergedAt: null,
          updatedAt: "2026-04-30T10:00:00Z",
        }),
      });
    ...
    it("does not let a same-second stray event un-merge a merged PR", async () => {
      // The canonical GitHub state remains merged even if a stray delivery
      // claims the PR reopened during the same second.
      ...
      const app = buildApp({
        pullRequestStateResolver: async () => ({
          ...
          merged: true,
          draft: false,
          title: "Merged current state",
          url: "https://github.com/Blockcast/paperclip/pull/4262",
          headSha: "merged-head",
          branch: "fix/blo-40022",
