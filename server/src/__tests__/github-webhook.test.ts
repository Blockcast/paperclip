...
describe("github-webhook pure helpers", () => {
  ...
  it("re-arms the lock probe with only the budget remaining after a delayed BEGIN", async () => {
    let now = 1_000;
    let actionCalled = false;
    let statementTimeoutMs: number | null = null;
    const positiveTimeouts: number[] = [];
    let callIndex = 0;
    ...
    const fakeReservedConnection = Object.assign(
      async (_strings: TemplateStringsArray, ..._values: unknown[]) => {
        ...
        if (callIndex === 1) {
          now += 30;
          return []; // begin consumes most of the request-wide 40ms budget
        }
        if (callIndex === 2) {
          now += statementTimeoutMs ?? 0;
      ...
      },
      {
        ...
        unsafe: async (sqlText: string) => {
          ...
          if (match) {
            statementTimeoutMs = Number(match[1]);
            if (statementTimeoutMs > 0) positiveTimeouts.push(statementTimeoutMs);
          }
          return [];
        },
      },
    ...
    try {
      await expect(
        __test_withPrReviewerTaskLock(
          fakeDb as never,
          "pr_review:Blockcast/paperclip:21582008",
          1_040,
          async () => {
            actionCalled = true;
            return "queued";
    ...
    expect(actionCalled).toBe(false);
    expect(positiveTimeouts).toEqual([40, 10]);
  });

  it("bounds fallback transaction setup before BEGIN and clears the reserved connection timeout", async () => {
    let readCalled = false;
    let released = false;
    let sawTimeoutBeforeBegin = false;
    let sawTimeoutReset = false;
    let statementTimeoutMs: number | null = null;
    const BEGIN_STALL_MS = 500;
    const fakeReservedConnection = Object.assign(
      async (strings: TemplateStringsArray) => {
        if (strings[0]?.trim() === "begin") {
          sawTimeoutBeforeBegin = statementTimeoutMs !== null && statementTimeoutMs > 0;
          const timeoutMs = statementTimeoutMs ?? Number.POSITIVE_INFINITY;
          return await new Promise((resolve, reject) => {
            const stalled = setTimeout(() => resolve([]), BEGIN_STALL_MS);
            setTimeout(() => {
              clearTimeout(stalled);
              reject(Object.assign(new Error("canceling statement due to statement timeout"), { code: "57014" }));
            }, timeoutMs);
    ...
    const fakeClient = {
      reserve: async () => fakeReservedConnection,
      options: { parsers: {}, serializers: {} },
    };

    const startedAt = Date.now();
    const result = await __test_boundedFallbackRead(
      fakeClient as never,
      Date.now() + 20,
      async () => {
        readCalled = true;
    ...
    expect(sawTimeoutBeforeBegin).toBe(true);
    expect(sawTimeoutReset).toBe(true);
    expect(released).toBe(true);
    expect(elapsedMs).toBeLessThan(BEGIN_STALL_MS / 2);
  });

  it("re-arms each fallback query with only the budget remaining after BEGIN", async () => {
    let now = 2_000;
    let statementTimeoutMs: number | null = null;
    const positiveTimeouts: number[] = [];
    let callIndex = 0;
    ...
    let result: unknown;
    try {
      result = await __test_boundedFallbackRead(
        fakeClient as never,
        2_040,
        async (_reservedDb, runQuery) =>
          runQuery(async () => {
            now += statementTimeoutMs ?? 0;
            throw Object.assign(new Error("canceling statement due to statement timeout"), { code: "57014" });
          }),
    ...
    expect(typeof result).toBe("symbol");
    expect(positiveTimeouts).toEqual([40, 15]);
  });
});
...
describeEmbeddedPostgres("github-webhook route", () => {
  ...
  // so the loss was invisible to the whole delivery funnel. These tests
  // reproduce genuine advisory-lock
  // contention across a second, independent DB connection (a fresh
  // `createDb(...)` pool -- `pg_advisory_xact_lock` only contends across
  // sessions) and assert the two outcomes the current retryable-contention
  // policy is meant to produce: a contention window shorter than the
  // request-wide lock budget self-heals, and one that outlasts it returns a
  // retryable 503 without dispatching outside the lock or inventing a
  // delivery-funnel outcome.
  describe("reviewer wake lock contention (BLO-21582)", () => {
    ...
    it(
      "returns a retryable 503 once lock acquisition is exhausted, without dispatching outside the lock",
      async () => {
        const reviewerAgentId = randomUUID();
        const { companyId } = await seedCompanyAndAgent();
        await db.insert(agents).values({
        ...
        // A lock timeout must ask GitHub to retry rather than answer 200 and
        // dispatch outside the lock. The delivery also never reached the
        // normal `received` increment inside the lock-guarded closure, so no
        // funnel outcome is recorded for this attempt.
        expect(res.status).toBe(503);
        expect(res.body).toMatchObject({
          error: "PR reviewer dispatch is contended; retry this webhook delivery",
          code: "pr_reviewer_dispatch_contended",
        });
        ...
        expect(afterReceived).toBe(beforeReceived);
        expect(afterDeadLettered).toBe(beforeDeadLettered);
    ...
    it(
      "bounds the lock-exhaustion fallback recheck instead of stalling behind the same blocked connection, " +
        "then returns a retryable 503 (BLO-21582 review follow-up: 'the fallback queries are outside the " +
    ...
    it(
      ...
      async () => {
        ...
        try {
          ...
          // after the retryable response had already been sent.
