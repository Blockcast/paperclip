...
describeEmbeddedPostgres("issue watchdog routes", () => {
  ...
  it("creates, updates, reads, lists, and removes an issue watchdog with activity logs", async () => {
    ...
    const watchdogActionNames = actionNames.filter((action) => action.startsWith("issue.watchdog_"));
    expect(watchdogActionNames).toHaveLength(3);
    expect(watchdogActionNames).toEqual(expect.arrayContaining([
