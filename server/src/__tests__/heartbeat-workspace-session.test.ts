...
describe("K8s session isolation metadata", () => {
  ...
  it("keeps the provisioned worktree when the isolation identity is precomputed by dispatch", () => {
    ...
    // sibling storage classes. `storage.workspace` is the ONLY field in the
    // descriptor that reads `usesEphemeralWorkspace`, so widening that
    // predicate moves it alone and cannot move these three: `home` and
    // `session` take the local `persistent` const, `cache` keys off
    // `isolationMode === "shared"`. That is exactly why they are asserted
    // here — they are the containment check on the line above, pinning that
    // the persistence stayed where it was put rather than spreading. Moving
    // any of them would take a different edit, in a different expression.
  ...
  // NOT about which predicate is consulted — `storage.workspace` is the only
  // reader of `usesEphemeralWorkspace` anywhere in the descriptor, so the
  // storage trio above is just as immune to that widening as these roots are.
  // It is about which PROPERTY is pinned: above, the storage *class* attached
  // to each root; here, the ephemeral root LAYOUT. Both derive from
  // `isolationMode`, and a layout change is worth catching on its own rather
  // than as a confusing second failure in the storage test.
