...
import {
  ...
  pullRequestWorkProductEventFingerprint,
...
describe("pullRequestWorkProductStatus", () => {
  ...
  it("uses authoritative current state when it differs from a triggering action", () => {
    expect(pullRequestWorkProductStatus({
      action: "synchronize",
      prState: "closed",
      prMerged: false,
    })).toBe("closed");
...
describe("pullRequestWorkProductSourceEventActionOrder", () => {
  it("retains a fallback order for legacy events without a source timestamp", () => {
...
describe("buildPullRequestWorkProductFields", () => {
  ...
  it("marks only an explicit ambiguous-state marker as reconciliation-pending", () => {
    expect(pullRequestWorkProductNeedsReconciliation({ sourceStateVerification: "pending" })).toBe(true);
    expect(pullRequestWorkProductNeedsReconciliation({ sourceStateVerification: "verified" })).toBe(false);
    expect(pullRequestWorkProductNeedsReconciliation(null)).toBe(false);
  });

  it("fingerprints source payload content without claiming it is an ordering key", () => {
    const closed = pullRequestWorkProductEventFingerprint({
      action: "closed",
      headSha: "head",
      prState: "closed",
    }, "closed");
    const reopened = pullRequestWorkProductEventFingerprint({
      action: "reopened",
      headSha: "head",
      prState: "open",
    }, "ready_for_review");
    expect(closed).not.toBe(reopened);
  });
});
