...
const mockCaptureAgentJobFailureDiagnostics = vi.hoisted(() => vi.fn(async () => null));
...
vi.mock("../services/k8s-job-liveness.ts", () => ({
  ...
  captureAgentJobFailureDiagnostics: mockCaptureAgentJobFailureDiagnostics,
...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("classifies a failed external app container as oom_killed and emits one failure metric", async () => {
    __resetMetricsForTest();
    const jobName = "agent-opencode-oom-killed";
    const { companyId, agentId, runId } = await seedRunFixture({
      adapterType: "opencode_k8s",
      processPid: null,
      processGroupId: null,
    ...
    await seedAdapterInvokeEvent({ companyId, agentId, runId });
    await seedLaunchedReservation({ companyId, agentId, runId, jobName, jobUid: "oom-killed-uid" });
    mockListManagedAgentJobs.mockResolvedValueOnce([{
      phase: "failed",
    ...
    mockCaptureAgentJobFailureDiagnostics.mockResolvedValueOnce({
      jobName,
      podName: `${jobName}-pod`,
      podPhase: "Failed",
      containers: [{
    ...
    try {
      ...
      const metricLines = (await renderMetrics()).body
        .split("\n")
        .filter((line) => line.includes('paperclip_heartbeat_run_failed_total{') && line.includes('error_code="oom_killed"'));
      expect(metricLines).toHaveLength(1);
      expect(metricLines[0]).toMatch(/ 1$/);

      await heartbeat.reapOrphanedRuns({ suppressDispatchAfterReap: true });
      const repeatedMetricLines = (await renderMetrics()).body
        .split("\n")
        .filter((line) => line.includes('paperclip_heartbeat_run_failed_total{') && line.includes('error_code="oom_killed"'));
      expect(repeatedMetricLines).toEqual(metricLines);
    } finally {
      __resetMetricsForTest();
    }
  });
  ...
  it("does not escalate a dependency-blocked continuation while a fresh blockers-resolved wake can still land", async () => {
    const { companyId, agentId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "cancelled",
      retryReason: "issue_continuation_needed",
    ...
    const issuePrefix = `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`;
    const freshDoneBlockerId = randomUUID();

    await db.insert(issues).values({
      id: freshDoneBlockerId,
      companyId,
      title: "Upstream work that just finished",
      status: "done",
      priority: "medium",
      issueNumber: 21,
  ...
  it("does not escalate a dependency-blocked continuation when finalization started before grace but completed recently", async () => {
    ...
    await db.insert(projects).values({
      id: projectId,
      companyId,
      name: "Dependency finalize project",
      status: "in_progress",
    });
    await db.insert(projectWorkspaces).values({
      id: projectWorkspaceId,
      companyId,
      projectId,
      name: "Dependency finalize workspace",
    ...
    await db.insert(executionWorkspaces).values({
      ...
      name: "Dependency finalize execution workspace",
    ...
    await db.insert(issues).values({
      ...
      companyId,
      projectId,
      title: "Upstream work that finalized after completion",
      status: "done",
      priority: "medium",
      issueNumber: 22,
  ...
  it("does not escalate a dependency-blocked continuation on the first recovery pass at the sweep threshold", async () => {
    const { companyId, agentId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "cancelled",
      retryReason: "issue_continuation_needed",
    ...
    const issuePrefix = `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`;
    const thresholdDoneBlockerId = randomUUID();

    await db.insert(issues).values({
      id: thresholdDoneBlockerId,
      companyId,
      title: "Upstream work at sweep threshold",
      status: "done",
      priority: "medium",
      issueNumber: 23,
    ...
    await db.insert(issueRelations).values({
      companyId,
      issueId: thresholdDoneBlockerId,
      relatedIssueId: issueId,
      type: "blocks",
    });
  ...
  it("does not escalate a dependency-blocked continuation while an exact dependency wake is still queued", async () => {
    ...
    await db.insert(issues).values({
      id: doneBlockerId,
      companyId,
      title: "Upstream work with emitted dependency wake",
      status: "done",
      priority: "medium",
      issueNumber: 24,
      identifier: `${issuePrefix}-24`,
      completedAt: new Date(Date.now() - 10 * 60 * 1000),
    });
    await db.insert(issueRelations).values({
      companyId,
