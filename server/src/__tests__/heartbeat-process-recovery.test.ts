...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("does not leave recovery artifacts when a concurrent manual block wins before claim", async () => {
    const { companyId, agentId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "failed",
      retryReason: "issue_continuation_needed",
    ...
    let resolveManualWrite!: () => void;
    let resolveManualCommit!: () => void;
    const manualWriteComplete = new Promise<void>((resolve) => {
      resolveManualWrite = resolve;
    });
    const allowManualCommit = new Promise<void>((resolve) => {
      resolveManualCommit = resolve;
    });
    const manualBlock = db.transaction(async (tx) => {
      await tx
        .update(issues)
        .set({
          status: "blocked",
      ...
      resolveManualWrite();
      await allowManualCommit;
    });

    await manualWriteComplete;
    const resultPromise = heartbeat.reconcileStrandedAssignedIssues();
    ...
    const issue = await db.select().from(issues).where(eq(issues.id, issueId)).then((rows) => rows[0] ?? null);
    expect(issue?.status).toBe("blocked");
    expect(await sourceBlockerIssueIds(companyId, issueId)).toEqual([blockerIssueId]);
