...
describe("maskPluginConfigJson", () => {
  ...
  it("collects discarded pointer payload leaves while retaining direct secret values", () => {
    const pointerPayloadSecret = "pointer-payload-live-secret";
    const directSecret = "direct-inline-live-secret";
    const schema = {
      type: "object",
      properties: {
    ...
    const config = {
      apiKeyRef: {
        ...
        secretId: SECRET_ID,
        version: "latest",
        value: pointerPayloadSecret,
      },
      webhookToken: directSecret,
      endpoint: "https://alerts.example.com",
    };

    const masked = maskPluginConfigJson(config, schema);
    expect(masked).toEqual({
      apiKeyRef: { type: "secret_ref", secretId: SECRET_ID, version: "latest" },
      webhookToken: PLUGIN_CONFIG_SECRET_MASK,
      endpoint: "https://alerts.example.com",
    ...
    const collected = collectPluginConfigSecretValues(config, schema);
    expect(collected).toEqual(expect.arrayContaining([pointerPayloadSecret, directSecret]));
    expect(collected).not.toContain(SECRET_ID);

    const merged = merge(config, {}, schema);
    expect(merged.configJson).toEqual({
      apiKeyRef: { type: "secret_ref", secretId: SECRET_ID, version: "latest" },
      webhookToken: directSecret,
      endpoint: "https://alerts.example.com",
    });
    expect(JSON.stringify(merged.configJson)).not.toContain(pointerPayloadSecret);
  });
