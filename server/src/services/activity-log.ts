 ...
 *
 * Never rejects: an enqueue failure is logged and swallowed, so callers cannot
 * lose an activity write (or abort an enclosing transaction) over the outbox.
 */
export async function publishPluginDomainEvent(event: PluginEvent, db?: Db | null): Promise<void> {
  ...
  try {
    await outboxDb.insert(pluginEventOutbox).values({
