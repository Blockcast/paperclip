...
- **A MIRRORED finding is dispositioned under EVERY head that reported it — its original and each head that re-stated it.** A finding's identity is `(head, severity, index)` and nothing links a mirror to its original: `extractAllyReportedFindingRefs` in `server/src/services/ally-review-detection.ts` mints refs from the `findings` *counts* alone, so re-stating an earlier finding in your own `Important Issues (2)` gives it a second, independent identity at *your* head. Dispositioning only the original leaves the mirroring head holding a finding no verb ever names — `isFullyDispositioned` in `server/src/services/pr-comment-review-gate.ts` requires every reported ref to be retired, so that one ref carries the whole head red until some later review attests the current one. Emit one entry per identity: `{"head":"195e6e2","severity":"important","index":1,"verb":"fixed"}` **and** `{"head":"ebae10f","severity":"important","index":1,"verb":"fixed"}`. Five clauses qualify that headline:
  - **WHICH HEADS get an entry — every link, not just the ends.** A chain is the steady state, not an edge case: a finding survives every push until it is fixed, and each surviving review re-states it, so H1 raises → H2 mirrors → H3 mirrors → H4 fixes needs one entry per head in the chain — three, not two. `headsWithUndispositionedFinding` in `server/src/services/pr-comment-review-gate.ts` scans *every* attesting head independently, so leaving out an intermediate head carries that head exactly as leaving out the mirror carries this one.
  - **WHAT THE RED TELLS YOU — one head, the newest one still carried.** So several omissions surface one review at a time. `evaluateCommentReviewGate` in `server/src/services/pr-comment-review-gate.ts` destructures the first element of that newest-first list and its reason string interpolates a single short SHA, so omitting both H2 and H3 reds on H3 alone; fix H3, re-review, and it reds again on H2. Audit the whole chain against your ledger rather than treating the head the status names as the only one outstanding — otherwise each omission costs its own review round, which is the tax this rule exists to remove.
  - **HOW `index` IS DERIVED — the finding's ordinal within that head's OWN review, so the two generally differ.** Recover the mirror's ordinal from the review that minted it rather than reusing the original's. Both read `1` above only because the mirror happened to lead `ebae10f`'s Important list; had `ebae10f` raised **another Important** ahead of the mirror, the mirror's entry would be `index:2` while the original's stayed `index:1`. Ordinals are namespaced per severity — `extractAllyReportedFindingRefs` numbers `1..count` *inside* a per-severity loop and `namesFinding` matches on severity *and* index — so a Critical ahead of the mirror does not shift it; only another finding of the mirror's own severity does.
  - **HOW `severity` IS DERIVED — it varies per entry too.** A finding re-graded between heads (raised Important at H1, escalated to Critical at H2) needs entries whose `severity` differs, not just whose `index` does.
  - **WHERE TO WRITE THE ORDINAL DOWN — when you mint it, not when you retire it.** Recovering it at the dispositioning review means re-counting an earlier body's bullets days later — the one step in the chain with no field behind it. At the *mirroring* review you already know it, because you are the one numbering that bucket: say so in the `still-present` ledger bullet you are already emitting for the original (`— still-present — mirrored here as important 1 —`). The later review then reads the ordinal off the ledger instead of reconstructing it. This is prose, not schema — omit it and you are back to today's behaviour, not a parse error.

  Two entries for one finding is legal — `asDispositions` in `server/src/services/ally-review-detection.ts` takes an arbitrary array and the gate retires per `(head, severity, index)` — so this needs no new field and no version bump. Measured on Blockcast/trafficcontrol#1707: the `ebae10f` review mirrored `prior:195e6e2 important 1`, the `78107bf4` review retired `195e6e2 important 1` and `ebae10f important 2`, and `ebae10f important 1` — the mirror — stayed carried.
