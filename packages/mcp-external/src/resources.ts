...
export function heartbeatRunResourceUris(runId: string) {
  return {
...
export async function readHeartbeatRunResource(client: PaperclipApiClient, uri: string): Promise<unknown> {
  ...
  if (kind === "log-chunks" && arg !== undefined) {
    ...
    return client.requestJson("GET", `/heartbeat-runs/${runId}/log`, { query: { offset, limitBytes } });
  }
  if (kind === "events" && parsed.suffix.length === 1) {
    ...
    return client.requestJson("GET", `/heartbeat-runs/${runId}/events`, { query: { afterSeq, limit } });
  }
