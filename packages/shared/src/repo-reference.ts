...
const REPO_CUE_PATTERN =
  /\b(repo|repos|repository|repositories|vendor|vendors|vendored|upstream|fork|forked|clone|cloned|checkout|monorepo|codebase|remote|submodule|mirror|github)\b/i;
...
export function extractRepoReferences(
  ...
  for (const pattern of [HTTPS_REPO_PATTERN, SSH_REPO_PATTERN]) {
    pattern.lastIndex = 0;
    let match: RegExpExecArray | null;
    while ((match = pattern.exec(text)) !== null) {
      record(makeReference(match[1], match[2], match[0], "url"));
    }
  }
  ...
  while ((backticked = BACKTICKED_PATTERN.exec(text)) !== null) {
    ...
    const accepted =
      knownOwners.has(owner.toLowerCase()) ||
      hasNearbyRepoCue(text, backticked.index, backticked.index + backticked[0].length);
