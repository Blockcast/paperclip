...
export const issueExecutionPolicySchema = z.object({
  ...
  monitor: issueExecutionMonitorPolicySchema
    ...
    .nullable()
    .describe(
      "The ONLY accepted way to arm, re-arm, or clear an issue monitor (wake) is this nested field, `executionPolicy.monitor`. There is no top-level `monitor` / `monitorNextCheckAt` / `monitorNotes` input — those keys are rejected, because they used to be silently discarded (BLO-18790). Carry a re-check signature in `monitor.notes`. An update REPLACES the whole `executionPolicy` rather than merging into it, so to clear a monitor you must read the issue's current policy and re-send it complete with only `monitor` omitted: a bare {\"executionPolicy\":{}} normalizes to null and erases `stages`, `reviewPreset` and `authorizationPolicy` along with the monitor. Re-arming supersedes a `triggered` monitor, which is how you reset a wedged one — but `attemptCount` is preserved across re-arm, so re-sending a `maxAttempts` at or below that count (or a `timeoutAt` already in the past) is rejected 422 as exhausted instead of re-arming; omit `maxAttempts` when resetting a wedged monitor. Monitors only hold on an `in_progress`/`in_review` issue assigned to an agent, and an unresolved `blockedBy` edge suppresses the wake even while it reads as `scheduled` — so ALWAYS re-read `monitorNextCheckAt` in the same run and treat `null` as failure rather than reporting success off a 200.",
    ),
...
export function misplacedIssueMonitorInputMessage(key: string) {
  return `\`${key}\` is not a writable issue field, so it would be silently discarded. Arm or re-arm a monitor with the nested shape \`executionPolicy.monitor\`, e.g. {"executionPolicy":{"monitor":{"nextCheckAt":"<ISO-8601>","notes":"<signature>","scheduledBy":"assignee"}}}. The \`monitor_*\` columns are server-owned: they are derived from that input and cannot be written directly. To clear a monitor, re-send the issue's complete current \`executionPolicy\` with only the \`monitor\` key omitted — the update replaces the whole policy, so a bare {"executionPolicy":{}} normalizes to null and would erase \`stages\`, \`reviewPreset\` and \`authorizationPolicy\` too.`;
}
