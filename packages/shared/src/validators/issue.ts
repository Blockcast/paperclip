  ...
  gateSignals: z
    ...
    .describe(
      "BLO-18294: the gates this monitor is actually waiting on, as short stable tokens (e.g. \"pr:Blockcast/paperclip#814:checks\", \"deploy:paperclip-api\"). Declaring them makes the convergence guard compare re-checks against THESE and ignore free-form `notes` churn, so an unrelated signal you happened to mention cannot read as progress and keep the loop alive. Unresolved `blockedBy` edges are folded in automatically — declare gateSignals for anything the issue graph does not already model. " +
        "BLO-22574: a `pr:<repo>#<n>:review` gate has TWO satisfying surfaces on GitHub, and `pulls/{n}/reviews` shows only one. Ally frequently answers as a plain PR comment headed `## Ally` and files no formal review object at all — `pulls/{n}/reviews` reads `reviews=0` forever on those PRs even though Ally has demonstrably reviewed (verified on Blockcast/magma#1655 and Blockcast/paperclip#929/#942/#948/#951/#952). Before re-arming on `reviews=0`, also check `issues/{n}/comments` for a body matching `^## Ally`; that comment carries a `Reviewed head: <40-hex>` line — compare it to the PR's current head for staleness, never the comment's timestamp. Only fall back to timestamp comparison when that line is genuinely absent. See `evaluatePrReviewGate` in `server/src/services/pr-review-gate.ts` for the reference implementation.",
    ),
