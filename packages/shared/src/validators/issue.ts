...
export const issueExecutionMonitorPolicySchema = z.object({
  ...
  gateSignals: z
    ...
    // not even the latest review; 0 of 94 COMMENTED re-anchored. Re-measured 2026-09-22 on
    // `pim-multicast-gateway#1968`: four APPROVED reviews of two different older heads
    // (`9b5e3a29…`, `11c80be5…`) all report one `commit_id=3168e6a2`, which is neither of them,
    // while their `Reviewed head:` body markers keep the two heads correctly distinct. This is the
    // fourth copy of the unsound recipe found (BLO-27234 `pim` CLAUDE.md, BLO-28737
    // `shaka-player/AGENTS.md`, BLO-34666 the agent bundle) and the only one no bundle write can
    // reach, so `issue.test.ts` guards it.
    .describe(
