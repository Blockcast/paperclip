...
export const issueExecutionMonitorPolicySchema = z.object({
  ...
  gateSignals: z
    ...
    .nullable()
    .default(null)
    // Schema-size note (BLO-32419): this description is ~3.7 KB (~940 tokens) and ships
    // in BOTH the create-issue and update-issue MCP tool schemas, so every agent pays it
    // roughly twice on every tool-schema load. The pagination clause below added ~660 of
    // those chars. Kept inline deliberately rather than reduced to a pointer at
    // `github-app-auth.ts`: this is the only copy an agent is guaranteed to have in
    // context at the moment it arms a monitor, and the failures it prevents — converging
    // a review gate that was in fact satisfied, or recording a verdict off a truncated
    // read — each cost whole runs. A pointer would be cheaper and would not be read.
    .describe(
