  ...
  gateSignals: z
    ...
    .describe(
      "BLO-18294: the gates this monitor is actually waiting on, as short stable tokens (e.g. \"pr:Blockcast/paperclip#814:checks\", \"deploy:paperclip-api\"). Declaring them makes the convergence guard compare re-checks against THESE and ignore free-form `notes` churn, so an unrelated signal you happened to mention cannot read as progress and keep the loop alive. Unresolved `blockedBy` edges are folded in automatically — declare gateSignals for anything the issue graph does not already model. " +
        "BLO-22574: a `pr:<repo>#<n>:review` gate token is opaque to the server — nothing evaluates it against GitHub, so YOU perform the re-check and record the verdict. It has TWO satisfying surfaces, and `pulls/{n}/reviews` shows only one. Ally frequently answers as a plain PR comment and files no formal review object at all — `pulls/{n}/reviews` reads `reviews=0` forever on those PRs even though Ally has demonstrably reviewed (verified on Blockcast/magma#1655 and Blockcast/paperclip#929/#942/#948/#951/#952). So `reviews=0` is NOT evidence of no review: before re-arming on it, also read `issues/{n}/comments`. Credit a comment only when it is authored by the reviewer App identity (`<slug>[bot]` or `app/<slug>` — a user-seat login of the same slug is a different actor), carries the canonical `## Ally — Consolidated PR Review` heading, and contains EXACTLY ONE standalone full-40-hex `Reviewed head:` attestation; zero or several means unproven, so fail closed rather than crediting it. Compare that attested SHA to the PR's current head for staleness — never the comment's timestamp, which cannot tell \"read this head\" apart from \"raced the push\". `githubHasReviewerEvidenceForPr` in `server/src/services/github-app-auth.ts` is the authoritative server-side implementation of exactly this check (it additionally credits an attested head that GitHub `compare` proves is a descendant of yours); mirror it rather than re-deriving weaker logic.",
    ),
