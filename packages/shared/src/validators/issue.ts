  ...
  gateSignals: z
    ...
    .describe(
      "BLO-18294: the gates this monitor is actually waiting on, as short stable tokens (e.g. \"pr:Blockcast/paperclip#814:checks\", \"deploy:paperclip-api\"). Declaring them makes the convergence guard compare re-checks against THESE and ignore free-form `notes` churn, so an unrelated signal you happened to mention cannot read as progress and keep the loop alive. Unresolved `blockedBy` edges are folded in automatically — declare gateSignals for anything the issue graph does not already model. " +
        "BLO-22574: a `pr:<repo>#<n>:review` gate token is opaque to the server, so YOU perform the GitHub re-check and record the verdict. Review evidence has TWO surfaces; `pulls/{n}/reviews` alone is incomplete. Keep this monitor evidence separate from a protected `review/ally-complete` merge requirement, which has its own formal-App approval rule. " +
        "SURFACE 1 — formal reviews (`pulls/{n}/reviews`): scan every entry because GitHub does not promise an order. A non-empty response is not enough. Treat a formal review from the configured reviewer App identity (`<slug>[bot]` or `app/<slug>`) at the PR's exact current `commit_id` as monitor evidence. The bare same-slug user seat is a different principal, never a substitute for App evidence or a protected merge approval. Do not invent an ad-hoc fallback for an off-head review or another author; the server recovery helper owns any separately verified recovery. Do not infer that an old review, comment, or non-approved review satisfies a protected merge gate; check that gate's exact-head formal-review policy separately. " +
        "SURFACE 2 — issue comments (`issues/{n}/comments`): Ally can leave a plain PR comment without a formal review, so `reviews=0` is NOT proof that no review happened. Before re-arming, also read issue comments. Credit a comment only when it is from the reviewer App identity, has the canonical `## Ally — Consolidated PR Review` heading, and contains EXACTLY ONE standalone full-40-hex `Reviewed head:` attestation that equals the current PR head. A bare same-slug user seat never counts on this surface. Zero, several, malformed, or stale attestations are unproven and fail closed; compare heads, never timestamps. Do not add a second evaluator — the server-side recovery helper is `githubHasReviewerEvidenceForPr` in `server/src/services/github-app-auth.ts`.",
    ),
...
export const suggestTasksResultSchema = z.object({
  ...
  // Set when the card is withdrawn before anyone acted on it. Mirrors the
  // `cancelled` / `cancellationReason` pair on askUserQuestionsResultSchema so
  // every withdrawable kind records the reason in the same shape.
...
export const requestConfirmationResultSchema = z.object({
  version: z.literal(1),
  // "cancelled" records a withdrawal by the creating agent (or a board user)
  // while the card was still pending. request_item_verdicts already carries the
  // same value, so the vocabulary stays consistent across kinds.
