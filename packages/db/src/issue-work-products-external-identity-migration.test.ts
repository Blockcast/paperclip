...
// Resolved by suffix rather than hardcoded: this migration has been renumbered
// twice by rebases (0208 -> 0211 -> 0212), and each time the stale literal here
// failed the suite with an ENOENT that looked unrelated to the rename.
const EXTERNAL_IDENTITY_MIGRATION_SUFFIX = "_issue_work_products_external_identity.sql";

const cleanups: Array<() => Promise<void>> = [];
const embeddedPostgresSupport = await getEmbeddedPostgresTestSupport();
const describeEmbeddedPostgres = embeddedPostgresSupport.supported ? describe : describe.skip;

function resolveExternalIdentityMigration(): string {
  const migrationsDir = new URL("./migrations/", import.meta.url);
  const matches = fs
    .readdirSync(migrationsDir)
    .filter((file) => file.endsWith(EXTERNAL_IDENTITY_MIGRATION_SUFFIX))
    .sort();
  if (matches.length !== 1) {
    throw new Error(
      `expected exactly one migration ending in ${EXTERNAL_IDENTITY_MIGRATION_SUFFIX}, found ${matches.length}: ${matches.join(", ")}`,
    );
  }
  return matches[0]!;
}

const EXTERNAL_IDENTITY_MIGRATION = resolveExternalIdentityMigration();
