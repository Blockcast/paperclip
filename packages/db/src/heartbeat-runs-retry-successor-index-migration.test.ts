/**
 * BLO-29312: migration 0230 must not build its index inline on a populated
 * heartbeat_runs table.
 *
 * Drizzle migrations are transactional, so CONCURRENTLY is unavailable and a
 * plain CREATE INDEX takes a SHARE lock on this large, high-write table for the
 * whole build, blocking every insert/update/delete against it. `IF NOT EXISTS`
 * makes a rerun idempotent but does nothing about that lock, so the migration
 * instead *requires* the index to be precreated online — and verifies the
 * precreated index actually matches the definition the retry-successor lookup
 * depends on, rather than accepting any index that happens to share the name.
 */
...
const MIGRATION_FILE = "0230_heartbeat_runs_retry_successor_index.sql";
const INDEX_NAME = "heartbeat_runs_retry_successor_idx";
...
describeEmbeddedPostgres("heartbeat-run retry successor index migration", () => {
  it("requires online precreation for a populated heartbeat_runs table", async () => {
    const database = await startEmbeddedPostgresTestDatabase("paperclip-heartbeat-retry-successor-index-");
  ...
  it("rejects an invalid precreated index with repair instructions", async () => {
    const database = await startEmbeddedPostgresTestDatabase("paperclip-heartbeat-retry-successor-invalid-");
    cleanups.push(database.cleanup);
    const sql = postgres(database.connectionString, { max: 1, onnotice: () => {} });
    cleanups.push(async () => sql.end());
  ...
  /**
   * A same-name index without the partial predicate would still answer the
   * lookup, so this case is about the index staying proportional to the retry
   * population rather than to all of heartbeat_runs — a silent acceptance here
   * is a silent table-sized index in production.
   */
  it("rejects a same-name index that is missing the partial predicate", async () => {
    const database = await startEmbeddedPostgresTestDatabase("paperclip-heartbeat-retry-successor-wrong-");
    cleanups.push(database.cleanup);
    const sql = postgres(database.connectionString, { max: 1, onnotice: () => {} });
    cleanups.push(async () => sql.end());
