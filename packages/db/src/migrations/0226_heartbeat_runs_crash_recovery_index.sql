...
--
-- The production runner applies each pending file in its own transaction (see
-- the runner described in 0225's header). A raise here therefore does not roll
-- back 0225's committed `ADD COLUMN`; this phase deliberately does not raise
-- when the index is absent because the index is a performance optimisation and
-- populated tables require an online build. An invalid same-named index may
-- raise safely: the repair needs nothing from this batch.
--
...
-- predeploy step, which is safe to run at any time after 0225 has committed:
--
...
BEGIN
  IF to_regclass('public.heartbeat_runs_crash_recovery_pending_idx') IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      RAISE EXCEPTION USING
        MESSAGE = 'migration 0226 found an invalid or incorrectly defined heartbeat_runs_crash_recovery_pending_idx',
  ...
  IF EXISTS (SELECT 1 FROM "heartbeat_runs" LIMIT 1) THEN
    RAISE NOTICE 'migration 0226: heartbeat_runs is populated; skipping inline index build. Create heartbeat_runs_crash_recovery_pending_idx online with CREATE INDEX CONCURRENTLY (see this migration''s header). Recovery is correct without it — the candidate scan degrades to a sequential scan.';
    RETURN;
  END IF;

  LOCK TABLE "heartbeat_runs" IN SHARE MODE;
  IF EXISTS (SELECT 1 FROM "heartbeat_runs" LIMIT 1) THEN
    RAISE NOTICE 'migration 0226: heartbeat_runs became non-empty while acquiring SHARE; skipping inline index build. Create heartbeat_runs_crash_recovery_pending_idx online with CREATE INDEX CONCURRENTLY (see this migration''s header).';
    RETURN;
  END IF;
