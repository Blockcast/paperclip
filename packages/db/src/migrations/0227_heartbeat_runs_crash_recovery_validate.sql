...
--
-- Split rationale in the header of 0225. The production runner applies each
-- pending file in its own transaction, so Phase A is already committed before
-- this validation runs. This file validates only what recovery is actually
-- *correct* against — the columns — and treats the index as reportable, not
-- required.
--
...
-- 0225 were ever edited into a form that silently no-ops (an `IF NOT EXISTS`
...
BEGIN
  ...
  IF invalid_columns IS NOT NULL THEN
    RAISE EXCEPTION USING
      MESSAGE = 'migration 0227: heartbeat_runs crash-recovery columns are missing or do not match the required shape after 0225: ' || invalid_columns,
      HINT = 'Phase A (0225) did not apply cleanly, or a pre-existing column survived its ADD COLUMN IF NOT EXISTS. Each column must have the stated type, be nullable, and carry no default; a default on crash_recovery_completed_at silently excludes every crash-marked run from recovery. Inspect heartbeat_runs, drop or ALTER the offending columns, then retry migrations.';
  END IF;
  ...
  -- only run once 0225 has committed — i.e. potentially after this migration.
  -- Raising here would not make the online build impossible: 0225 is already
  -- committed by the per-file migration runner. The index remains optional.
  index_present := to_regclass('public.heartbeat_runs_crash_recovery_pending_idx') IS NOT NULL;
  IF NOT index_present THEN
    RAISE NOTICE 'migration 0227: heartbeat_runs_crash_recovery_pending_idx is absent. Crash recovery is correct without it (the candidate scan degrades to a sequential scan); create it online when convenient — see 0226.';
  END IF;
END
$$;
