...
--
-- Split rationale in the header of 0218. In short: drizzle runs every pending
...
-- 0218 were ever edited into a form that silently no-ops (an `IF NOT EXISTS`
...
BEGIN
  ...
  IF invalid_columns IS NOT NULL THEN
    RAISE EXCEPTION USING
      MESSAGE = 'migration 0220: heartbeat_runs crash-recovery columns are missing or do not match the required shape after 0218: ' || invalid_columns,
      HINT = 'Phase A (0218) did not apply cleanly, or a pre-existing column survived its ADD COLUMN IF NOT EXISTS. Each column must have the stated type, be nullable, and carry no default; a default on crash_recovery_completed_at silently excludes every crash-marked run from recovery. Inspect heartbeat_runs, drop or ALTER the offending columns, then retry migrations.';
  END IF;
  ...
  -- only run once 0218 has committed — i.e. potentially after this migration.
  -- Raising here would roll 0218 back and make that step impossible.
  index_present := to_regclass('public.heartbeat_runs_crash_recovery_pending_idx') IS NOT NULL;
  IF NOT index_present THEN
    RAISE NOTICE 'migration 0220: heartbeat_runs_crash_recovery_pending_idx is absent. Crash recovery is correct without it (the candidate scan degrades to a sequential scan); create it online when convenient — see 0219.';
  END IF;
END
$$;
