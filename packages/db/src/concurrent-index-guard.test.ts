/**
 * BLO-21526: migration 0219 records complete on a populated `heartbeat_runs`
 ...
 * state 0219 leaves behind, it must build the index and fail loudly — never
...
async function seedPopulatedDatabaseWithoutIndex() {
  ...
  // Applying migrations against an empty database builds 0219's index
  ...
  // migration 0219 itself leaves behind on a real populated deploy.
...
describeEmbeddedPostgres("ensurePendingConcurrentIndexes", () => {
  it("builds the deferred index migration 0219 leaves absent on a populated table", async () => {
  ...
  it("throws — rather than reporting success — when the online build does not leave a valid index", async () => {
    ...
    const brokenSpec: ConcurrentIndexSpec = {
      migration: "0219_heartbeat_runs_crash_recovery_index.sql",
      name: INDEX_NAME,
  ...
  it("throws instead of silently trusting a same-named index with a different definition (BLO-21526 review)", async () => {
    ...
    await expect(ensurePendingConcurrentIndexes(database.connectionString)).rejects.toThrow(
      /does not match migration 0219_heartbeat_runs_crash_recovery_index\.sql's definition/,
    );
