...
const MIGRATION_FILE = "0233_alertmanager_aggregate_creation_dedupe.sql";
...
describeEmbeddedPostgres("Alertmanager aggregate index migration", () => {
  it("requires exact online precreation for a populated issues table", async () => {
    ...
    await expect(
      ...
      message: "migration 0233 requires online index precreation",
  ...
  it("rejects an incorrectly defined same-name index", async () => {
    ...
    await expect(
      ...
      message:
        "migration 0233 found an invalid or incorrectly defined prerequisite index",
      hint: expect.stringContaining(
