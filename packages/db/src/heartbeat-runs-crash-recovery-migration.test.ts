 ...
 *
 * The predecessor combined the column addition and the online-index path in
 * one migration. This split must be exercised from a genuinely pre-phase-A
 * database, so it proves that the columns become durable before an operator
 * needs the concurrent index.
 *
 * The previous test could not see any of that: it started from a database that
 * was already post-0212 (the column present, only the index dropped), which is
 ...
 * genuinely pre-0212 shape.
 */
const PHASE_A = "0212_heartbeat_runs_crash_recovery_columns.sql";
const PHASE_B = "0213_heartbeat_runs_crash_recovery_index.sql";
const PHASE_C = "0214_heartbeat_runs_crash_recovery_validate.sql";
...
/**
 * Rewind the database to the pre-0212 shape: drop the columns and index the
 * three phases install, and forget that they ever ran.
 */
async function rewindToPre0212(sql: postgres.Sql) {
  await sql.unsafe(`
...
describeEmbeddedPostgres("heartbeat-run crash-recovery migration phases", () => {
  ...
  it("applies cleanly to a populated pre-0212 database without requiring the index first", async () => {
    ...
    await rewindToPre0212(sql);

    expect(await presentColumns(sql)).toEqual([]);

    // Must NOT throw: a missing performance index cannot block the durable
    // recovery state or the later online CREATE INDEX CONCURRENTLY step.
  ...
  it("rejects an incorrectly defined same-name index with a repair hint", async () => {
    ...
    await expect(applyPendingMigrations(database.connectionString)).rejects.toMatchObject({
      message: `migration 0213 found an invalid or incorrectly defined ${INDEX_NAME}`,
  ...
  it("keeps phase A durable when 0213 raises on a genuinely pre-0212 database, so the repair hint is followable", async () => {
    // Review round 3 argued that 0213's malformed-index RAISE rolls phase A
    ...
    //
    // Setup is genuinely pre-0212 — no crash_recovery_* columns at all. The
    // malformed index therefore cannot reference one, so it is a same-name
    // index over a predicate that is buildable without them.
    const { database, sql } = await freshDatabase("paperclip-crash-recovery-pre0212-malformed-");
    await seedOneRun(sql);
    await rewindToPre0212(sql);
    expect(await presentColumns(sql)).toEqual([]);
    await sql.unsafe(`
    ...
    expect(failure, "expected 0213 to reject on the malformed index").not.toBeNull();
    expect(failure?.message).toBe(`migration 0213 found an invalid or incorrectly defined ${INDEX_NAME}`);

    // The load-bearing fact: phase A committed in its own transaction and the
    // 0213 raise did not take it with it.
    expect(await presentColumns(sql)).toEqual([...CRASH_COLUMNS].sort());
  ...
  it("rejects a same-type crash-recovery column that carries a default", async () => {
    ...
    await expect(applyPendingMigrations(database.connectionString)).rejects.toMatchObject({
      message: expect.stringContaining("migration 0214"),
