 ...
 * precreate an index whose predicate referenced that column. The production
 * migration runner applies one file per transaction, so the repair hint is
 * followable after the column phase commits; migrations still could not advance
 * until the index was repaired.
 *
 * The previous test could not see any of that: it started from a database that
 * was already post-0225 (the column present, only the index dropped), which is
 ...
 * genuinely pre-0225 shape.
 */
const PHASE_A = "0225_heartbeat_runs_crash_recovery_columns.sql";
const PHASE_B = "0226_heartbeat_runs_crash_recovery_index.sql";
const PHASE_C = "0227_heartbeat_runs_crash_recovery_validate.sql";
...
/**
 * Rewind the database to the pre-0225 shape: drop the columns and index the
 * three phases install, and forget that they ever ran.
 */
async function rewindToPre0225(sql: postgres.Sql) {
  await sql.unsafe(`
...
describeEmbeddedPostgres("heartbeat-run crash-recovery migration phases", () => {
  ...
  it("applies cleanly to a populated pre-0225 database without requiring the index first", async () => {
  ...
  it("rejects an incorrectly defined same-name index with a repair hint", async () => {
    ...
    await expect(applyPendingMigrations(database.connectionString)).rejects.toMatchObject({
      message: `migration 0226 found an invalid or incorrectly defined ${INDEX_NAME}`,
  ...
  it("keeps phase A durable when 0226 raises on a genuinely pre-0225 database, so the repair hint is followable", async () => {
    // The production path (`pnpm db:migrate` -> packages/db/src/migrate.ts)
    // applies each file in its own transaction and commits its history row
    // before starting the next. This test pins that operator-facing contract:
    // Phase A must remain durable when Phase B rejects a malformed index, so the
    // hinted online repair can be followed immediately.
    //
    // Setup is genuinely pre-0225 — no crash_recovery_* columns at all. The
    // malformed index therefore cannot reference one, so it is a same-name
    // index over a predicate that is buildable without them.
    const { database, sql } = await freshDatabase("paperclip-crash-recovery-pre0225-malformed-");
    await seedOneRun(sql);
    await rewindToPre0225(sql);
    expect(await presentColumns(sql)).toEqual([]);
    await sql.unsafe(`
    ...
    expect(failure, "expected 0226 to reject on the malformed index").not.toBeNull();
    expect(failure?.message).toBe(`migration 0226 found an invalid or incorrectly defined ${INDEX_NAME}`);

    // The load-bearing fact: phase A committed in its own transaction and the
    // 0226 raise did not take it with it.
    expect(await presentColumns(sql)).toEqual([...CRASH_COLUMNS].sort());
  ...
  it("rejects a same-type crash-recovery column that carries a default", async () => {
    ...
    await expect(applyPendingMigrations(database.connectionString)).rejects.toMatchObject({
      message: expect.stringContaining("migration 0227"),
