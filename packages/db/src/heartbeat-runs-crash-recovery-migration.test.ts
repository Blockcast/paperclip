 ...
 * was already post-0213 (the column present, only the index dropped), which is
 ...
 * genuinely pre-0213 shape.
 */
const PHASE_A = "0213_heartbeat_runs_crash_recovery_columns.sql";
const PHASE_B = "0214_heartbeat_runs_crash_recovery_index.sql";
const PHASE_C = "0215_heartbeat_runs_crash_recovery_validate.sql";
...
/**
 * Rewind the database to the pre-0213 shape: drop the columns and index the
 * three phases install, and forget that they ever ran.
 */
async function rewindToPre0213(sql: postgres.Sql) {
  await sql.unsafe(`
...
describeEmbeddedPostgres("heartbeat-run crash-recovery migration phases", () => {
  ...
  it("applies cleanly to a populated pre-0213 database without requiring the index first", async () => {
  ...
  it("rejects an incorrectly defined same-name index with a repair hint", async () => {
    ...
    await expect(applyPendingMigrations(database.connectionString)).rejects.toMatchObject({
      message: `migration 0214 found an invalid or incorrectly defined ${INDEX_NAME}`,
  ...
  it("keeps phase A durable when 0214 raises on a genuinely pre-0213 database, so the repair hint is followable", async () => {
    // Review round 3 argued that 0214's malformed-index RAISE rolls phase A
    ...
    //
    // Setup is genuinely pre-0213 — no crash_recovery_* columns at all. The
    // malformed index therefore cannot reference one, so it is a same-name
    // index over a predicate that is buildable without them.
    const { database, sql } = await freshDatabase("paperclip-crash-recovery-pre0213-malformed-");
    await seedOneRun(sql);
    await rewindToPre0213(sql);
    expect(await presentColumns(sql)).toEqual([]);
    await sql.unsafe(`
    ...
    expect(failure, "expected 0214 to reject on the malformed index").not.toBeNull();
    expect(failure?.message).toBe(`migration 0214 found an invalid or incorrectly defined ${INDEX_NAME}`);

    // The load-bearing fact: phase A committed in its own transaction and the
    // 0214 raise did not take it with it.
    expect(await presentColumns(sql)).toEqual([...CRASH_COLUMNS].sort());
  ...
  it("rejects a same-type crash-recovery column that carries a default", async () => {
    ...
    await expect(applyPendingMigrations(database.connectionString)).rejects.toMatchObject({
      message: expect.stringContaining("migration 0215"),
