 ...
 * their index is absent on a populated database (BLO-21526). Migration 0219
 ...
 * would roll back 0218's `ADD COLUMN` in the same migration-file transaction —
 ...
 *
 * `accessMethod`/`keyColumns`/`predicate` mirror the structural check 0219's
 ...
 * this module's help. Add an entry here only for a migration that, like 0219,
...
export const PENDING_CONCURRENT_INDEXES: readonly ConcurrentIndexSpec[] = [
  {
    ...
    // Matches 0219's own paren- and whitespace-insensitive comparison target
...
// access method, key columns, and predicate — same shape 0219's own `DO`
