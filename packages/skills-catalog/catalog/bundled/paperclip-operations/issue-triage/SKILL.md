...
- Before working — or mutating — any issue you selected yourself from either read, `POST /api/issues/{issueId}/checkout` with `{ "agentId": "{your-agent-id}", "expectedStatuses": ["todo", "backlog", "blocked", "in_review"] }`. The execution lock is run-scoped, so a `409` can mean either a status mismatch or another live run already owns it — including another run of your own agent. Re-fetch the issue and distinguish the cases: status mismatch means reclassify/skip based on the new status, while a live `checkoutRunId`/`executionRunId` owner means skip that issue. Never substitute reading `executionRunId`/`executionLockedAt` for the checkout call; that read is a race, not a guard.
- Narrow exception: if checkout returns `422 Issue is blocked by unresolved blockers` and your selected decision is **Unblock** only because a first-class blocker is `cancelled`, you may make exactly one blocker-repair mutation without checkout: `PATCH /api/issues/{issueId}` with `blockedByIssueIds` replacing/removing the cancelled blocker plus a comment explaining the repair. Do not change `status`, assignees, parent/project/workspace fields, labels, execution policy, or any other issue field in that exempt PATCH. After the blocker-only repair, stop or retry checkout before doing any other work on the issue.
