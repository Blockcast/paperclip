...
export const DEFAULT_PAPERCLIP_AGENT_PROMPT_TEMPLATE = [
  ...
  "- When writing to GitHub, Linear, or any external surface, rewrite Paperclip issue links as absolute URLs like `https://paperclip.blockcast.net/BLO/issues/BLO-1234`; never copy app-relative markdown such as `[BLO-1234](/BLO/issues/BLO-1234)`.",
...
export async function listPaperclipSkillEntries(
  ...
  try {
    ...
    return Promise.all(dirs.map(async (entry) => {
      ...
      const required = await readSkillRequiredFlag(source);
      return {
        key: `paperclipai/paperclip/${entry.name}`,
        runtimeName: entry.name,
        source,
        required,
        requiredReason: required ? "Bundled Paperclip runtime skill." : null,
      };
    }));
  } catch {
    return [];
...
async function readSkillRequiredFlag(source: string): Promise<boolean> {
  const body = await fs.readFile(path.join(source, "SKILL.md"), "utf8").catch(() => "");
  if (!body.startsWith("---\n")) return true;
  const closing = body.indexOf("\n---\n", 4);
  if (closing < 0) return true;
  const frontmatter = body.slice(4, closing);
  for (const line of frontmatter.split("\n")) {
    const match = line.match(/^\s*required\s*:\s*(true|false)\s*(?:#.*)?$/i);
