...
describe("sweepOrphanedRunSecrets", () => {
  ...
  // BLO-21857 review follow-up (PR #1459, allyblockcast[bot]): the reviewer's
  // objection was that the age floor did not bound the launch window, so age
  // alone could authorise deleting the Secrets of a launch still in flight. The
  // floor is now sized to that window (DEFAULT_SWEEP_AGE_FLOOR_SEC, 15 min)
  // rather than to the happy path, which is what makes age sufficient. A
  // renewable lease was tried instead and removed: it let a cleanup path abort
  // healthy launches. The second half of the objection — that one Job-list
  // snapshot can be stale by the time the delete runs — is answered by the
  // pre-delete re-read, tested below.
  it("does not collect the Secrets of a launch wedged past the old 120s floor", async () => {
    // A launch stalled in a slow or retrying K8s API call between
    // createNamespacedSecret and createNamespacedJob. Nothing in execute.ts
    // bounds that gap, so this Secret is ownerless with no Job and would have
    // been swept under the previous 120s floor.
    const h = harness([secret("ac-agent-run-slow-prompt", { runId: "run-slow", ageSec: 600 })]);

    const result = await sweepOrphanedRunSecrets(h.opts);

    expect(result.swept).toEqual([]);
    expect(result.retained).toEqual([
      { name: "ac-agent-run-slow-prompt", reason: "too_young" },
    ]);
    expect(h.deleteNamespacedSecret).not.toHaveBeenCalled();
