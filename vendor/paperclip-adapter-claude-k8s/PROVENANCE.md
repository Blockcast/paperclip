...
| [PEN-2955](https://paperclip.blockcast.net/PEN/issues/PEN-2955) | `src/server/job-manifest.ts`, `src/server/job-manifest.test.ts` | Cut the default agent-run memory **request** 2Gi → 1.5Gi (the 8Gi limit is unchanged). Measured across 38 pods: 536 Mi mean, 1.79 GiB max — a 3.7x over-request stranding ~55.6 GiB cluster-wide, on a fleet already confined to 2 of 10 nodes ([PEN-2943](https://paperclip.blockcast.net/PEN/issues/PEN-2943)). Sized to the tail rather than the mean because the request is the only admission control here; the limit absorbs bursts. The change is only worth anything if the *default* is what reaches the pod, and that was verified rather than assumed: `mergeEnvironmentConfig` is a **top-level** merge, so the `resources` block that `onprem-k8s/paperclip/penstock-agent-environment-reconciler.yaml` writes into the `penstock-general` environment row — nested `{requests:{cpu:500m,memory:1Gi},limits:{cpu:4,memory:16Gi}}` — never satisfies the dotted `config["resources.requests.memory"]` lookup and is inert. Confirmed live with a positive control: the Security Engineer pod runs on that row (SA `penstock-general-agent`, so the row IS merging) yet reported `requests 1/2Gi, limits 4/8Gi` — the code defaults, not the row's values, with the 8Gi-vs-16Gi limit as the discriminator. A new test pins that lookup so the reclamation cannot be silently undone. The row's dead `resources` block is a real latent bug but belongs to `onprem-k8s`, so it is reported separately, not fixed here. |
