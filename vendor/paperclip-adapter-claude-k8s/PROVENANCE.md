...
```
aa037ef0b5f72194ccdfdd4ac392fdd1587335bcf5fe907b55abbd2dd7dae20f
```
...
| [BLO-22546](https://paperclip.blockcast.net/BLO/issues/BLO-22546) | `src/server/job-manifest.ts`, `src/server/job-manifest.test.ts` | Inverted `adapterConfig.env` classification from default-ALLOW to default-DENY. Operator-set values were routed to the per-run Secret only when the variable's NAME matched `/(TOKEN\|SECRET\|PASSWORD\|KEY\|CREDENTIAL\|AUTH)/i`; anything else rendered as a literal `value:`, readable by any principal with `get pods`. The fleet-wide zero that produced was load-bearing on naming luck — `FIGMA_API_KEY` matched on `KEY`, `WEBFLOW_BOT_CONTROL_TOKEN` on `TOKEN`; rename either and the credential returns to the PodSpec with no error, no failing test and no alert. Measured against the shipped regex, 17 of 22 ordinary credential names render as literals, including `VERCEL_BEARER`, `GCP_SA_JSON`, `PGPASSFILE` and every `*_PAT` form. Operator-set keys are now Secret-backed because of WHERE THEY CAME FROM rather than what they are called, so an unrecognized key fails safe. Platform-computed values (`HOME`, `TMPDIR`, `PAPERCLIP_RUN_ID`) stay readable literals — a closed set defined in this file, carrying no credential material — pinned by a negative-control test so that "Secret-ify everything", which would satisfy the criterion while destroying `GET Pod` debuggability, cannot pass in its place. The fail-closed backstop `findLiteralSensitiveEnvVarsInPodSpec` now takes the operator key set too, since keying it on the same name predicate left it blind to exactly the class default-deny exists to stop. The stricter BLO-22514 server-only deny still wins over this path: `DATABASE_URL` supplied through `adapterConfig.env` fails the build outright rather than being laundered onto the pod as a `secretKeyRef`, which the kubelet would still resolve into the container. Closes AC-3, the one acceptance criterion a fleet sweep cannot verify — a sweep measures today's names, AC-3 governs tomorrow's. |
