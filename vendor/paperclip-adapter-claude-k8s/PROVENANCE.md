...
```
ae16d98210e5675b7618bf363ce05d99c2c07c614db3255c71e5847dbf5f571a
```
...
| [BLO-22546](https://paperclip.blockcast.net/BLO/issues/BLO-22546) | `src/server/job-manifest.ts`, `src/server/job-manifest.test.ts` | Made `adapterConfig.env` **default-deny**, closing AC 3 of BLO-22546. `SENSITIVE_ENV_NAME_RE` is a name *pattern*, so an operator key the pattern does not match — `PENSTOCK_BOARD`, `GH_PAT`, `*_CRED`, `ANTHROPIC_CUSTOM_HEADERS`, or a plain `FOO_BAR` — shipped as a literal `value:` readable by any principal with `get pods`, which is a broader grant than `get secrets` and the whole point of the Secret/PodSpec split. Operator-supplied env is a small map that is overwhelmingly credential material, so at that layer classification now ignores the key name entirely; the broad pattern still covers the merged/inherited layers, per the 2026-08-22 CTO ruling on the issue. Scoped to the keys whose operator value *survived* the merge (`merged[k] === envConfig[k]`), because Layer 4 is applied before the session-header/HOME/isolation/cache blocks and several of those overwrite unconditionally — where the adapter won, the shipped value is the adapter's own non-secret path and correctly stays a literal. `DOCKER_HOST` is excluded via `ADAPTER_REASSERTED_ENV_NAMES` since `enableDocker` re-asserts it as a literal *after* `buildEnvVars` returns; without that exclusion an operator setting the same key would get both a `secretKeyRef` and a literal for one name and the new guard would fire on the adapter's own write. The `&& value` guard is load-bearing: an empty operator value must stay inline, or the pod fails at startup with `CreateContainerConfigError` on a `secretKeyRef` to a key `sensitiveEnvData` never receives. Adds a fail-closed `findLiteralOperatorEnvVarsInPodSpec` backstop beside the two existing ones, checked on the assembled spec so a container added later cannot escape it — this is what makes the default-deny non-regressable rather than a convention a future edit quietly undoes. Three pre-existing tests asserted the old contract (operator env ships as a literal `.value`) and were updated to assert Secret-backed delivery; operator *precedence* is unchanged in all three, only the delivery mechanism. Verified: 802/802 tests, `tsc --noEmit` clean, build clean, and the new default-deny falsifier confirmed to FAIL against a name-pattern-only classifier. Does **not** close BLO-22546: `opencode_k8s` is still cloned from `kkroo` at `OPENCODE_K8S_REF` and remains unfixed. |
