...
| Current version | `0.2.6-blockcast.3` — see [Versioning](#versioning) |
...
```
58b8074e24bb192d8d1a035a04d1a066676ce74c95d121838a9421aa35e2319f
```
...
| [BLO-21857](https://paperclip.blockcast.net/BLO/issues/BLO-21857) | `src/server/secret-sweep.ts` (new), `src/server/secret-sweep.test.ts` (new), `src/server/execute.ts`, `src/server/execute.test.ts` | Closed the ownerless-Secret window between `createNamespacedSecret` and the `ownerReferences` patch. Every ordinary path either sets that owner reference or deletes the Secret, but a control-plane crash in the window between the two leaves a Secret with no owner — and K8s GC, including the Job `ttlSecondsAfterFinished` cascade, only collects through recorded owner references, so it survives indefinitely. Since `35f1eb2`/`6ddd4b0` these Secrets carry the credential material [BLO-17973](https://paperclip.blockcast.net/BLO/issues/BLO-17973) was filed about, so an orphan is a credential resident in etcd, not litter. Not hypothetical: the sibling `paperclip-adapter-opencode-k8s`, same pattern, had **63 of 70** prompt Secrets ownerless at up to ~76 days ([BLO-22110](https://paperclip.blockcast.net/BLO/issues/BLO-22110)). Adds a best-effort sweep, rate-limited by a module-level interval gate and piggybacked on `execute()` because `ServerAdapterModule` exposes no lifecycle hook to hang a timer on. Deliberately conservative: it acts only on Secrets that are ours by label, have zero `ownerReferences`, are past both an age floor and a **launch lease**, and have no owning Job by **either** the run-id label **or** the `<jobName>-<suffix>` name convention, re-confirmed by a point read of that Job immediately before the delete. The lease is the load-bearing guard and replaces an earlier revision's reliance on the age floor alone, which rested on a false premise: that entry claimed a 15s `K8S_CONCURRENCY_GUARD_TIMEOUT_MS` bounded the create-to-create gap, but that guard wraps only the pre-launch Job lookup — the three Secret creates and the Job create are bare awaits, so nothing bounds how long a launch may sit Secret-created-but-Job-less. `execute.ts` therefore stamps `paperclip.io/launch-expires-at` on each Secret at creation and the sweep refuses to touch one whose lease is unexpired (or unparseable), which holds **across control-plane replicas** as a process-local interval gate cannot. The pre-delete point read closes the complementary hole: a Job created after the sweep's list snapshot would otherwise be invisible. Absence must be proven — a read that succeeds, fails for any reason other than 404, or names a Secret from which no Job name can be derived all retain the Secret. Both guards were mutation-checked. Raised on Ally's review of [#1459](https://github.com/Blockcast/paperclip/pull/1459), which identified live-credential loss during a slow or concurrent launch as the cost of getting this wrong. That last check is doubled because the Secret's run-id label is the raw `runId` while the Job's is `sanitizeLabelValue(runId)`, which can differ or be omitted entirely; they agree for every Secret that can exist, but the name-based check does not depend on that inference, so a future labelling change degrades the sweep into leaving orphans (visible, safe) rather than deleting live Secrets. The three Secret-creation sites now take their labels from the same exported constants the sweep selects on, so creation and collection cannot drift apart. One Job list per sweep, not one per Secret. Worst-case orphan lifetime falls from unbounded to the launch lease (default 900s) + one sweep interval (~20 min at defaults) — deliberately traded against never collecting a Secret a live launch still needs. |
...
This directory therefore versions itself, starting at **`0.2.6-blockcast.1`**
and set in `package.json` and `package-lock.json`. The `-blockcast.` prerelease
channel says plainly that this is our tree, not an upstream release. The current
value is in the [Origin](#origin) table above, and CI keeps the two in step.
