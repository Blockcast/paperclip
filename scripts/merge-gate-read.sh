...
# fixture to merge-gate-read.test.mjs; it has shipped four false-GREENs.
#
...
verdicts() { # $1 = DEAD alternation ('__none__' when nothing is stale)
  ...
  grep -vE "	(${dead})$" \
    ...
    | awk -F'\t' -v dead="$dead" '$4!="status"&&$2!="neutral"{n++}
        ...
        END{if(!n && dead!="__none__") print "STOP\t<every check-run at this head dropped as superseded-run>\tABSENT\trun=-"}'
}
