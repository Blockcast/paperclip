...
const workflowFiles = (await readdir(workflowsDir))
  ...
  .sort();
const ALLOWED_RUNNERS = new Set(["default", "arc-light", "arc-dind", "arc-deploy", "arc-e2e", "arc-merge-queue"]);

// BLO-22428: a handful of heavy jobs route merge_group traffic to the
// dedicated arc-merge-queue pool while pull_request traffic stays on
// `default`, via `runs-on: ${{ <cond> && 'X' || 'Y' }}`. This checker
...
const TERNARY_RUNNER_EXPRESSION = /^\$\{\{[\s\S]*?&&\s*'([^']*)'\s*\|\|\s*'([^']*)'\s*\}\}$/;
const violations = [];
