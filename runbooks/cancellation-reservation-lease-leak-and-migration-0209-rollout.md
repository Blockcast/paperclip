...
scheduler-interval cycles, or a scheduler/worker pod crash-looping on
startup while applying database migrations.
...
5. **Migration online-index prerequisites are now automatic.**
   ...
   pending that requires an online-precreated index (0205, 0208, 0209 today
   — see `ONLINE_INDEX_PREREQUISITES`), it: precreates the index with
...
documents for its alert family, before trusting this alert as live.
