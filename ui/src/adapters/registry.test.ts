...
describe("ui adapter registry", () => {
  ...
  it("restores the external-package Hermes local adapter when overrides are paused or removed", () => {
    const type = "hermes_local";
  ...
  it("removes the external Hermes gateway UI adapter when paused or removed", () => {
    const type = "hermes_gateway";
    expect(findUIAdapter(type)).toBeNull();

    syncExternalAdapters([{ type, label: "External Hermes Gateway" }]);
    expect(findUIAdapter(type)).toMatchObject({
      type,
      label: "External Hermes Gateway",
      ConfigFields: SchemaConfigFields,
    });

    syncExternalAdapters([{ type, label: "External Hermes Gateway", overrideDisabled: true }]);
    expect(findUIAdapter(type)).toBeNull();

    syncExternalAdapters([{ type, label: "External Hermes Gateway" }]);
    expect(findUIAdapter(type)).not.toBeNull();
